...
# ──────────────────────────────────────────────────────────────────
# 1. COLLECTE DES CLÉS API (saisie masquée)
# ──────────────────────────────────────────────────────────────────

echo "━━━ ÉTAPE 1/5 — Clés API LLM (saisie masquée) ━━━━━━━━━━━━━━━"
echo ""
...
read -rsp "  HUGGINGFACE_API_KEY : " HF_KEY
echo " ✓"
echo ""
...
echo "  ✅ backend/.env (permissions 600)"
...
EOF
chmod 600 "$INFRA_ENV"
echo "  ✅ infrastructure/.env (permissions 600)"
...
if [ -n "$(git status --porcelain)" ]; then
  git add -A
  git commit -m "chore: update config" 2>/dev/null || true
...
chmod 600 .env
echo '  ✅ Clés injectées (permissions 600)'
...
# ──────────────────────────────────────────────────────────────────
# 5. VÉRIFICATION (sans afficher les clés)
# ──────────────────────────────────────────────────────────────────
...
for p in json.load(sys.stdin):
    ok = 'CONNECTE' if p['configured'] else 'NON CONFIG'
    print(f\"  {p['id']:15s} {ok:15s} {p['default_model']}\")
"
